...
export async function deriveAllKeys(masterKey: CryptoKey): Promise<DerivedKeys> {
  const encryptionKey = await subtle().deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: HKDF_INFO_ENCRYPT },
    masterKey,
  ...
  const authKey = await subtle().deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: HKDF_INFO_AUTH },
    masterKey,
  ...
  const memorySaltBits = await subtle().deriveBits(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: HKDF_INFO_SALT },
    masterKey,
    256
  );
