...
export function resetRemoteSyncAuthState(userId?: string): void {
  if (!userId) { blockedRemoteSyncUsers.clear(); return; }
...
function deserializeModel(value: string | null | undefined): SelectedModel {
  if (!value) return ROUTER_SELECTED_MODEL;
  try {
  ...
  return { id: value, name: value.split('/').pop()?.replace(/[-_]/g, ' ') || 'Saved Model', provider: value.split('/')[0] || 'Unknown', runtime: 'puter' };
}
...
export async function fetchRemoteConversations(userId: string, defaultSettings: ChatSettings): Promise<Conversation[]> {
  ...
  const { data: conversations, error } = await insforge.database
    .from('conversations').select('id,user_id,title,model,created_at,updated_at')
    .eq('user_id', userId).order('updated_at', { ascending: false });
  if (error) {
    if (isMissingSyncSchemaError(error)) return [];
    if (isUnauthorizedSyncError(error)) { blockRemoteSync(userId); return []; }
    throw error;
  }
  ...
  const { data: messages, error: msgError } = await insforge.database
    .from('messages').select('id,conversation_id,role,content,created_at')
    .in('conversation_id', conversationIds).order('created_at', { ascending: true });
  if (msgError && !isMissingSyncSchemaError(msgError)) {
    if (isUnauthorizedSyncError(msgError)) { blockRemoteSync(userId); return []; }
    throw msgError;
  }
  ...
  return (conversations as ConversationRow[]).map(conv => {
    const convMessages: Message[] = (msgsByConv.get(conv.id) || []).map(m => ({
      id: m.id, role: m.role, content: m.content, timestamp: toTimestamp(m.created_at, now),
    }));
    return {
      id: conv.id, title: conv.title || 'Untitled', messages: convMessages,
      model: deserializeModel(conv.model), settings: { ...defaultSettings },
      createdAt: toTimestamp(conv.created_at, now), updatedAt: toTimestamp(conv.updated_at, now),
      source: 'remote' as const,
    };
  });
}

export async function pushConversationToRemote(userId: string, conversation: Conversation): Promise<void> {
  if (shouldSkipRemoteSync(userId)) return;
  try {
    const { error: convError } = await insforge.database.from('conversations').upsert({
      id: conversation.id, user_id: userId, title: conversation.title || 'Untitled',
    ...
    if (convError) {
      ...
      if (isUnauthorizedSyncError(convError)) { blockRemoteSync(userId); return; }
      throw convError;
    }
    if (conversation.messages.length === 0) return;
    const messageRows = conversation.messages.map(m => ({
      id: m.id, conversation_id: conversation.id, role: m.role,
      content: m.content, created_at: new Date(m.timestamp).toISOString(),
    }));
    const { error: msgError } = await insforge.database.from('messages').upsert(messageRows, { onConflict: 'id' });
    if (msgError) {
    ...
    if (isUnauthorizedSyncError(err)) { blockRemoteSync(userId); return; }
    throw err;
  }
}

export async function deleteRemoteConversation(userId: string, conversationId: string): Promise<void> {
  if (shouldSkipRemoteSync(userId)) return;
  try {
    await insforge.database.from('messages').delete().eq('conversation_id', conversationId);
    await insforge.database.from('conversations').delete().eq('id', conversationId).eq('user_id', userId);
  } catch (err) {
    if (isMissingSyncSchemaError(err)) return;
    if (isUnauthorizedSyncError(err)) { blockRemoteSync(userId); return; }
    throw err;
